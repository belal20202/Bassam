...
export const MainMenu: React.FC<MainMenuProps> = ({
  ...
  const activeTitleObj = TITLES_LIST.find((t) => t.id === playerData.activeTitleId) || TITLES_LIST[0];
  ...
  const activeMissions = playerData.missions.filter((m) => m.category === 'DAILY' || m.category === 'DAILY_CHALLENGES');
  const claimableCount = dailyChallenges.filter((c) => c.isCompleted && !c.isClaimed).length +
    activeMissions.filter((m) => m.isCompleted && !m.isClaimed).length;

  return (
    <div id="main-menu-screen" className="absolute inset-0 z-20 flex flex-col justify-between p-3.5 sm:p-5 bg-gradient-to-b from-slate-950/75 via-slate-900/30 to-slate-950/85 backdrop-blur-[2px] overflow-y-auto">
      {/* Top Bar: Player Level, Title & IQD Balance */}
      <div className="flex flex-wrap items-center justify-between gap-2.5 w-full max-w-4xl mx-auto">
        {/* Level & Active Title */}
        <div className="flex items-center gap-2">
          {/* Level Box */}
          <div className="flex items-center gap-2 bg-slate-900/90 border border-amber-500/30 px-2.5 py-1.5 rounded-xl shadow-lg backdrop-blur-md">
            {/* The single yellow badge containing ONLY 'المستوى' as requested */}
            <div className="flex items-center justify-center px-2.5 h-7 rounded-lg bg-gradient-to-r from-amber-500 to-yellow-300 text-slate-950 font-black text-xs shadow-sm">
              <span>المستوى</span>
            </div>
            <div className="flex flex-col min-w-[100px]">
              <div className="flex justify-between items-baseline text-[10px] font-bold text-slate-200 gap-1.5">
                <span className="text-amber-300 font-mono font-bold text-xs">{playerData.level}</span>
                <span className="text-slate-400 font-mono text-[9px]">
                  {playerData.xp.toLocaleString('en-US')}/{currentLevelReqXP.toLocaleString('en-US')}
                </span>
              </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-slate-900/90 border border-amber-500/40 hover:border-amber-400 px-2.5 py-1.5 rounded-xl shadow-lg backdrop-blur-md transition-all group"
          >
            <span className="text-sm group-hover:scale-110 transition-transform">
              {activeTitleObj.badge}
            </span>
            <span className="text-[11px] font-bold text-amber-300 group-hover:text-white transition-colors">
              {activeTitleObj.name}
            </span>
      ...
      <div className="w-full max-w-md mx-auto grid grid-cols-4 gap-2">
        ...
        >
          ...
          <span className="text-[11px] font-bold text-slate-200">المهام</span>
        </button>
